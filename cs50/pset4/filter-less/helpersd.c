...
RGBTRIPLE copy[height][width]; // Create new array 'copy' made up of 'RGBTRIPLE' struct types, to store altered pixels and allow mods without alterating original pixels
                ...
                for (j = 0; j < width; j++)
                    ...
                    {   // Blue pixels
                        ...
                        if ((test_i_zero + 1) >= 0 && (test_i_height + 1) <= (height - 1) && (test_j_zero + 1) >= 0 && (test_j_zero + 1) <= (width - 1))
                            ...
                            copy[i][j].rgbtBlue = round(holder / counter);  // should be 'counter' variable, but can't get it to count yet and so delivers a zero which creates a floating point problem.,so '9' fo rnow.
                        }
    ...
    // copy the modified pixels back into original
    int i = 0;
    int j = 0;
