...
person *create_family(int generations)
{
    ...
    int pcounter = 0; // ##counter, for print check. may not be necessary


    // Start the list, by declaring a a pointer to an empty list, of NULL value.
    // This will be the header for the list later.

    // Header node? I don't think this is necessary as you create the person elsewhere.
    person *p = NULL;
   ...
   // Using malloc to declare dynamic memory for a temporary person struct, called 'temp'.
    ...
    // #test. manually entering values of 'p' node.
    {
        temp->alleles[0] = 'A';
        temp->alleles[0] = 'B';
        temp->parents[0] = NULL;
...
person *q = NULL;
    ...
    if (generations > 1)
    {
        ...
        for (i=0; i > 2; i ++)
        {
            ...
            while (temp->alleles[i] == 0)
            {
                char ran_allele = random_allele(); // creates random char of A, B or O, assigns to 'ran_allele'
                printf("ran_allele = %c\n", ran_allele);  //check print
                if ((i = 0 && (ran_allele == (parent0->alleles[0] || parent0->alleles[1]))) || (i = 1 && ((ran_allele == (parent1->alleles[0] || parent1->alleles[1])))))
                    {
                        temp->alleles[i] = ran_allele;
                        printf("temp->alleles[%i]: %c\n", i, temp->alleles[i]);
                    }
            }
        }
    }
    ...
    person *newperson = malloc(sizeof(person));    // Using malloc to declare dynamic memory for a temporary person struct, called 'new person'.

    if (newperson == NULL) // check mem for null
    {
        printf("Not enough memory. Program aborted\n");
        return 0;
    }

    newperson = temp; // copy temp into 'new person', but how do I disintguish this new person from others?
    pcounter++;
    printf("Person created, no: %i\n", pcounter);
...
//            if (ran_allele == (parent0->alleles[0] || parent0->alleles[1]))
  //              {
    //                temp->alleles[0] = ran_allele;
      //              printf("temp->alleles[0]: %c\n", temp->alleles[0]);
     //   }
