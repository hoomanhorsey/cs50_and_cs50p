...
// prints hashes for a number determined by the (h) and the (r). i.e. if h = 8, then for row 1, (y = (8-1); 7 <8+1; y++), only 1 hash is printed before 'x < h + 1' becomes false, and printing stops.
...
// Loop B4 calculates how many # on each row, based on (r)ow, as per same loop in B2
                {
                    int z;
                    {
                        for (z = (h - r); z < (h + 1); z++)
