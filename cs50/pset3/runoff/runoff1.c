...
{
    // DEFINITIONAL
    // 'Preferences array' keeps track of voters + their votes by preference
    // 'Candidate struct' keeps track of candidate name, number of votes they have, and whether they are eliminated.
...
typedef struct
{
    ...
    int candidateindex[candidate_count];
    //note, could i make this an array to include structs? So I can include candidate count and names?

    for (int a = 0; a < candidate_count; a++)
    {
        candidateindex[a] += 1; // for each candidate, assign a new number for each candidate in array
    }
...
// "candidates[i].name = argv[i + 1];"
    ...
    for (int i = 0; i < voter_count; i++) // looping for each voter
    {
        ...
        {
            preferences[i][j] = candidateindex[i];
